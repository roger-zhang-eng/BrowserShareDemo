...
class ShareViewController: SLComposeServiceViewController, ColorSelectionViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        for content in extensionContext!.inputItems as! [NSExtensionItem] {
            for provider in content.attachments as! [NSItemProvider] {
                
                if provider.hasItemConformingToTypeIdentifier(contentType) {
                    print("Meet kUTTypePropertyList condition!")
                    provider.loadItem(forTypeIdentifier: contentType, options: nil, completionHandler: { (jsDict, error) -> Void in
                        DispatchQueue.main.async(execute: {
                            if jsDict != nil {
                                let jsPreprocessingResults = jsDict as! NSDictionary
                                let pageTitle = jsPreprocessingResults["title"] as? String
                                print("pageTitle: \(pageTitle)")
                                let urlString = jsPreprocessingResults["URL"] as? String
                                print("URL: \(urlString)")
                            }
                        })
